...
$requiredFields = array("byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid");
sort($requiredFields);
foreach ($inputs as $input) {
    $parts = preg_split('/\s+/', $input);
    $checkedParts = array();
    foreach ($parts as $part) {
        $partsToCheck = explode(":", $part);
        if (count($partsToCheck) !== 2) {
            continue;
        }
        $checkedParts[] = $partsToCheck[0];
    }
    sort($checkedParts);
    if (count(array_intersect($requiredFields, $checkedParts)) == count($requiredFields) && validatePassport($parts)) {
        $validPassports++;
    } else {
        echo "Passport denied!\n";
        var_dump($parts);
    }
}
...
function validatePassport($passportData) {
    ...
    foreach ($passportData as $passportDatum) {
        $partsToCheck = explode(":", $passportDatum);
        // $requiredFields = array("byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid");
        if (count($partsToCheck) !== 2) {
            continue;
        }
        
        switch($partsToCheck[0]) {
            case "byr":
                $isValidPassport = validateBirthYear($partsToCheck[1]);
                break;
            case "iyr":
                $isValidPassport = validateIssueYear($partsToCheck[1]);
                break;
            case "eyr":
                $isValidPassport = validateExpirationYear($partsToCheck[1]);
                break;
            case "hgt":
                $isValidPassport = validateHeight($partsToCheck[1]);
                break;
            case "hcl":
                $isValidPassport = validateHairColor($partsToCheck[1]);
                break;
            case "ecl":
                $isValidPassport = validateEyeColor($partsToCheck[1]);
                break;
            case "pid":
                $isValidPassport = validatePassportId($partsToCheck[1]);
                break;
            case "cid":
                break;
            default:
                echo "Invalid field " . $partsToCheck[0] . " detected";
                break;
        }
        if (!$isValidPassport) {
...
function validateIssueYear($issueYear) {
    if (!is_numeric($issueYear)) {
        return false;
    }
    
    $issueYear = intval($issueYear);
    echo "Issue Year: " . $issueYear . "\n";
    return $issueYear >= 2010 && $issueYear <= 2020;
}

function validateExpirationYear($expirationYear) {
    if (!is_numeric($expirationYear)) {
        return false;
    }
    
    $expirationYear = intval($expirationYear);
    
    echo "Expiration Year: " . $expirationYear . "\n";
    return $expirationYear >= 2020 && $expirationYear <= 2030;
}

function validateHeight($height) {
    $measuringSystem = substr($height, -2);
    if ($measuringSystem == "cm") {
        $height = intval(substr($height, 0, -2));
        return $height >= 150 && $height <= 193;
    } else if ($measuringSystem == "in") {
        $height = intval(substr($height, 0, -2));
...
function validateHairColor($hairColor) {
    $colorInfo = explode("#", $hairColor);
    if (count($colorInfo) !== 2) {
        return false;
    }
    $colorInfo = $colorInfo[1];
    return preg_match('/[0-9a-f]{6}/', $colorInfo);
}

function validateEyeColor($eyeColor) {
    $legalEyeColors = array("amb", "blu", "brn", "gry", "grn", "hzl", "oth");
    return in_array($eyeColor, $legalEyeColors);
}

function validatePassportId($passportId) {
    return is_numeric(intval($passportId)) && strlen($passportId) == 9;
}
